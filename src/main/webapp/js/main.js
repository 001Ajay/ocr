(function() {
	...
	function setupVideo(rearCameraId) {
		var deferred = new $.Deferred();
		var getUserMedia = getUserMedia = Modernizr.prefixed('getUserMedia',
				navigator);
	...
	function isMobile() {
		...
		return (a.match(/Android/i) || a.match(/BlackBerry/i)
				|| a.match(/iPhone|iPad|iPod/i) || a.match(/Opera Mini/i) || a
				.match(/IEMobile/i));
	}
	...
	function flipImage(img, ctx, width, height, flipH, flipV) {
		...
		ctx.drawImage(img, (flipH ? width * -1 : 0), (flipV ? height * -1 : 0),
				width, height);
		ctx.restore();
		return ctx;
	}
	...
	function step2() {
		...
		if (isMobile()) {
			// Flipping image upside down due to firefox bug
			flipImage(video, ctx, pictureWidth, pictureHeight, false, true);
		} else {
		...
		fxCanvas.draw(texture).hueSaturation(-1, -1)//grayscale
		.unsharpMask(20, 2).brightnessContrast(0.2, 0.9).update();
	...
	function step3() {
		...
		var ctx = canvas.getContext('2d');
		ctx.drawImage(step2Image, cropData.x * scale, cropData.y * scale,
				cropData.w * scale, cropData.h * scale, 0, 0, cropData.w
						* scale, cropData.h * scale);

		//use ocrad.js to extract text from the canvas
		var resultText = OCRAD(ctx);
		resultText = resultText.trim();
		var targetId = $('#modal-popup').data('target');
		console.log('TargetID : ' + targetId);
		$('#' + targetId).val(resultText);
	...
	function changeStep(step) {
		...
		$('.nav li:eq(' + (step - 1) + ')').removeClass('disabled').addClass(
				'active');
	}
	...
	$('#brightness, #contrast').on(
			...
			function() {
				...
				fxCanvas.draw(texture).hueSaturation(-1, -1).unsharpMask(20, 2)
	...
	$('.opop').click(function(e) {
		e.preventDefault();
		console.log(e.target);
		var tgt = $(e.target).parent().attr('target');
		console.log('tgt : ' + tgt);
		var p = $('#modal-popup');
		p.show();
		p.data('target', tgt);
